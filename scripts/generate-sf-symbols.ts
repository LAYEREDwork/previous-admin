...
function generateIndexFile(iconsDir: string, iconNames: string[]): void {
  ...
  const exports = sortedNames
    .map(name => `export const SF${name} = createSFIcon('${name}');`)
    .join('\n');
...
async function generateSFIcons() {
  ...
  if (svgFiles.length === 0) {
    console.log('⚠️  No SVG files found in frontend/components/sf-symbols/svgs directory.');
    return;
  }
