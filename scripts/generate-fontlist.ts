...
function generateFontList() {
  try {
    ...
    if (sortedFonts.length === 0) {
      console.warn('⚠️  Could not extract any font names from files');
      return;
    }
...
`;
    ...
    console.log('✅ Generated font list:', OUTPUT_FILE);
    console.log('📝 Available fonts:', sortedFonts.join(', '));
  } catch (error) {
    console.error('❌ Error generating font list:', error);
    process.exit(1);
  }
}
