...
- **`:cm`**: Erstelle eine kompakte Commit-Message in Englisch, für die Änderungen seit dem letzten Commit. Es sollen nur aktuell geänderte Dateien berücksichtigt werden (`git status`). Die Message soll eine Überschrift (Headline) und eine ungeordnete Liste enthalten und als reiner Text in einer Code-Box formatiert sein. Es soll kein Commit ausgefuehrt werden!
