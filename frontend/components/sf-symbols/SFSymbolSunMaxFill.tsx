interface SFSymbolSunMaxFillProps {
  /**
...
/**
 * SunMaxFill Icon Component
 *
 * A custom SVG icon from SF Symbols.
 * Follows the same API conventions as React Icons library components.
 *
 * @param {SFSymbolSunMaxFillProps} props - Component props
 * @returns {JSX.Element} The SVG icon element
 *
 * @example
 * // Basic usage
 * <SFSymbolSunMaxFill size={24} color="currentColor" />
 *
 * @example
 * // With custom styling
 * <SFSymbolSunMaxFill size={32} color="#ff0000" className="custom-class" />
 */
export function SFSymbolSunMaxFill({
  size = 24,
  color = 'currentColor',
  className = '',
  strokeWidth = 1,
  ...rest
}: SFSymbolSunMaxFillProps): JSX.Element {
  const numSize = typeof size === 'string' ? parseInt(size, 10) : size;

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 27.4805 27.1973"
      width={numSize}
      height={numSize}
      fill={color}
