...
import { PASizeEnum, PABasicSize } from '../../lib/types/sizes';
...
export function PASkeuomorphButton({
  ...
  shape = ButtonShape.pill,  containerBg = 'auto',  onClick,
  className = '',
  title,
  ...
  const colors = color ? colorConfig[color] : defaultColors;

  // Shape-specific corner radius
  const shapeRadius = {
    pill: 'rounded-full',
    rect: size === 'xs' ? 'rounded-[7px]' : size === 'sm' ? 'rounded-[8px]' : size === 'md' ? 'rounded-[10px]' : 'rounded-[12px]',
    round: 'rounded-full',
  }[shape];

  // Inner padding based on shape (round needs equal padding for square aspect)
  const innerPadding = shape === 'round' ? config.roundInner : config.inner;

  // Surface-specific styling
  const getSurfaceStyle = () => {
    const baseBoxShadow = surface === 'flat'
  ...
  return (
    ...
    >
      ...
      >
        ...
        {shape !== 'round' && children && (
          <span
            className={`font-medium ${config.text} transition-all duration-200 ${contentStyle} -translate-y-px`}
            style={textShadowStyle}
          >
            {children}
          </span>
