...
export function PASkeuomorphButton({
  ...
  const cornerRadius = shape === PASkeuomorphButtonShape.rect ? buttonHeight / 4 : buttonHeight;
  ...
  const noiseTexture = "url(\"data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3e%3cfilter id='n'%3e%3cfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3e%3c/filter%3e%3crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.03'/%3e%3c/svg%3e\")";

  // Color constants for styles
  const buttonColorShadowDark  = '#222';                        // dark drop shadow (bottom right)
  const buttonColorShadowLight = 'rgba(255, 255, 255, 0.25)';   // light highlight (top left)
  const frameColorShadowDark   = '#000';                        // dark frame shadow
  const frameColorShadowLight  = 'rgba(255, 255, 255, 0.2)';    // light frame highlight
  const gradientColorLight     = 'rgba(255,255,255,0.08)';      // gradient top
  const gradientColorDark      = 'rgba(0,0,0,0.12)';            // gradient bottom

  // Tailwind classes for frame, ring, button
  // Softer light and shadow for frame and button
  // Dynamic box-shadow for frame and ring using frameWidth/ringWidth
  const frameShadow = `inset ${frameWidth}px ${frameWidth}px ${frameWidth * 2}px ${frameColorShadowDark}, inset -${frameWidth}px -${frameWidth}px ${frameWidth}px ${frameColorShadowLight}`;
  ...
  // Dynamic box-shadow for button using buttonBorderWidth
  // Correct light (top left) and shadow (bottom right)
  // Correct order: light (top left), shadow (bottom right)
  const buttonShadow = `-${buttonBorderWidth}px -${buttonBorderWidth}px ${buttonBorderWidth}px ${buttonColorShadowLight}, ${buttonBorderWidth}px ${buttonBorderWidth}px ${buttonBorderWidth * 2}px ${buttonColorShadowDark}`;
  const buttonActiveShadow = `inset -${buttonBorderWidth}px -${buttonBorderWidth}px ${buttonBorderWidth}px ${buttonColorShadowLight}, inset ${buttonBorderWidth}px ${buttonBorderWidth}px ${buttonBorderWidth * 2}px ${buttonColorShadowDark}`;
  const buttonClass = `w-full h-full font-semibold border-none transition-all duration-200 ease-in-out focus:outline-none bg-next-panel disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none disabled:bg-next-panel`;
          ...
          style={{
            ...
            boxShadow: !disabled
              ? (active
                  ...
                  : buttonShadow)
              : undefined,
            // Use noiseTexture constant and combine with gradient
            backgroundImage: `linear-gradient(${gradientColorLight},${gradientColorDark}), ${noiseTexture}`,
          }}
        >
          <span
